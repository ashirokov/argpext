...
with exdoc.Workdir('examples') as W:
    ...
    for program,filename in [
        ...
        exdoc.interp(program,filename)


    for script,value in [['sheepgraze.py', [(['-h'],'sheepgraze_help.tmp')
                         ...
                         [os.path.join(W.init_dir,'argpext.py'), [(['-h'], 'argpext_help.tmp')]]]:

        for args,outputfile in value:

            exdoc.scriptrun(sys.executable,script,args,outputfile)
